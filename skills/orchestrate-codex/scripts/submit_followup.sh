...
end_position=$(awk -v marker="$end_marker" '{ position = index($0, marker); if (position) last = offset + position; offset += length($0) + 1 } END { if (last) print last }' "$after_first")
accepted_position=$(awk -v marker="$accepted_marker" '{ position = index($0, marker); if (position) last = offset + position; offset += length($0) + 1 } END { if (last) print last }' "$after_first")
if [[ -n $end_position && -n $accepted_position ]] && ((end_position < accepted_position)); then
  echo "dispatch was accepted but the clean composer has not returned; no second Enter sent; inspect $evidence_dir" >&2
  exit 1
fi
